...
var page = new Vue({
    ...
    watch: {
        ...
        frequentlyAddress:function () {
            if(this.frequentlyAddress===true){
    ...
    methods: {
        ...
        judgeAddress:function () {
            ...
            else if (this.receiverName.length>5&&re3.test(this.receiverName)){
                this.mBox("最多5个汉字");
            }
            else if(this.receiverName.length>20&&re2.test(this.receiverName)){
                this.mBox("最多20个英文字母");
            }
            else if(this.receiverName>20){
                this.mBox("长度超出限制");
            }
            else if(this.detail===""){
                this.mBox("请输入街道地址");
            }
            else if(this.receiverTel===""){
                this.mBox("请输入手机号");
            }
            else if(!re.test(this.receiverTel)){
                this.mBox("请输入正确的手机号");
            }
            else{
                if(this.tab==='6_1')
        ...
        addAddress:function () {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    city: that.city,//城市
                    county: that.district,//区
                    detail:that.detail,//详细地址
                    receiverTel:that.receiverTel,//收货人电话
                    postalCode:that.postalCode,//邮政编码
                    status:that.addressStatus//是否为常用地址
                },
                dataType: "json",
                contentType:'application/x-www-form-urlencoded; charset=UTF-8',
                success: function (msg) {
        ...
        //点击修改地址按钮后的数据变化
        selected:function () {
                this.receiverName=this.selectedAddress.receiverName;
                this.prov=this.selectedAddress.province;
                this.city=this.selectedAddress.city;
                this.district=this.selectedAddress.county;
                this.detail=this.selectedAddress.detail;
                this.receiverTel=this.selectedAddress.receiverTel;
                this.postalCode=this.selectedAddress.postalCode;
                this.addressStatus=this.selectedAddress.status;

                if(this.addressStatus===0){
                    this.frequentlyAddress=false;
                }
                else{
                    this.frequentlyAddress=true;
                }
